...
{	
	...
	try
	{ 
        if (moleculeFormula.size() == 0 || moleculeFormula.size() > MAX_FORMULA_LENGTH)
		{
		...
		if (retVal == SUCCESS)
		{
			...
			while (iterator != NULL && retVal == SUCCESS)
			{
				...
				if (search == periodicTableOfElements.end())
				{
                    ...
                    SetLastError(string(tmpBufferErrorMsg));
				}
        ...
        *elementSummary = string(eSummary);
	}
    catch (exception ex)
	{
...
map<string, double> FormulaParser::ParseElements(int *errorCode)
{
	...
	try
	{
		...
		if (formula != NULL && calculatedMoleMass > .5)
		{
			...
			while (iterator != NULL)
			{
                ...
                stlMap.insert(std::pair<string, double>(string(iterator->element_symbol), elementComposition));
				iterator = iterator->next;
			}
		}
		else
